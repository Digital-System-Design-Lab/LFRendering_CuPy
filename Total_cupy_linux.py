...
percent1,memory1=GPUInfo.gpu_usage()
...
def LoadLFs5K():
    ...
    for i in range(0,len(LF)):
        LF[i] = cp.zeros(3 * Params.LFU_W * Params.HEIGHT * Params.WIDTH * 3 , 'uint8')
        for n in range(1,Params.LFU_W * 3+1):
            ...
            """
            LF[i][(nn) * (Params.HEIGHT * Params.WIDTH * 3) + (TX) * (Params.HEIGHT * 3) + (TY) * 3 + 3 -1] = cp.asarray(img_C1[:])
            LF[i][(nn) * (Params.HEIGHT * Params.WIDTH * 3) + (TX) * (Params.HEIGHT * 3) + (TY) * 3 + 2 -1] = cp.asarray(img_C2[:])
            LF[i][(nn) * (Params.HEIGHT * Params.WIDTH * 3) + (TX) * (Params.HEIGHT * 3) + (TY) * 3 + 1 -1] = cp.asarray(img_C3[:])
            """
...
def RenderingUserViewLF_AllinOne5K(LF=None, LFDisparity=None, FB=None, viewpoint=None, DIR=None):
    ...
    if (FB == 1):
        ...
        ANGn = ANGn + CENTERx
        Pn = (Params.LFU_W / 2 - viewpoint.pos_y) * cp.tan(ANGn) + viewpoint.pos_x + (3 * Params.LFU_W / 2)
    elif (FB == 2):
        ANGn = cp.cos(ANGy) * cp.arctan(-TX / r) 
        ANGn = ANGn - CENTERx
        Pn = (Params.LFU_W / 2 + viewpoint.pos_y) * cp.tan(ANGn) + viewpoint.pos_x + (3 * Params.LFU_W / 2)
...
test1,test2=GPUInfo.gpu_usage()
print("test :"+str(test1) + "%, " + str(test2) + "MB")
...
for n in range(0,Params.LFU_W * 3): 
    nn = n - 1
    imageflow = cp.transpose(np.expand_dims(LFDisparity_forward[n,:,:],axis=0), (1,2,0)) 
    imageflow = cp.squeeze(imageflow, axis=2)
...
#문제의 지점
#LFDisp_forward = cp.rint((LFDisp_forward*10000))
#cp.rint((LFDisp_forward*10000))
...
for n in range(0,Params.LFU_W * 3): 
    nn = n - 1
    imageflow = cp.transpose(np.expand_dims(LFDisparity_backward[n,:,:],axis=0), (1,2,0)) 
    imageflow = cp.squeeze(imageflow, axis=2)
    imageflow = imresize(imageflow,output_shape=(Params.HEIGHT, Params.WIDTH))
    imageflow = (Params.HEIGHT / Params.RESIZE_HEIGHT) * imageflow
    imageflow = imageflow.T
...
#문제확인
#LFDisp_backward = cp.rint((LFDisp_backward*10000))
...
print("Loaded Backward Depth...\n")


D2 = time.time()


percent2,memory2=GPUInfo.gpu_usage()
...
percent3,memory3=GPUInfo.gpu_usage()
...
percent4,memory4=GPUInfo.gpu_usage()

print("start :"+str(percent1) + "%, " + str(memory1) + "MB")
print("Depth End :"+str(percent2) + "%, " + str(memory2) + "MB")
print("Color End :"+str(percent3) + "%, " + str(memory3) + "MB")
print("Rendering End :"+str(percent4) + "%, " + str(memory4) + "MB")

"""
OutViewF_cpu = cp.asnumpy(OutViewF)
OutViewB_cpu = cp.asnumpy(OutViewB)
"""
"""
cv2.imwrite('OutViewF.jpg',cp.uint8(OutViewF_cpu))
cv2.imwrite('OutViewB.jpg',cp.uint8(OutViewB_cpu))
"""

"""
Img.imsave('OutViewF.jpg',cp.uint8(OutViewF_cpu))
Img.imsave('OutViewB.jpg',cp.uint8(OutViewB_cpu))
"""
"""
OutViewF = cp.uint8(OutViewF)
OutViewB = cp.uint8(OutViewB)
"""
...
OutFlowF_cpu = cp.asnumpy(OutFlowF)
OutFlowB_cpu = cp.asnumpy(OutFlowB)


scipy.io.savemat('OutFlowF.mat',{'OutFlowF':OutFlowF_cpu})
scipy.io.savemat('OutFlowB.mat',{'OutFlowB':OutFlowB_cpu})
"""
t2 = time.time()


print("Reading Depth time : " + str(D2 - D1) + "s")

print("Reading Color time : " + str(L2 - L1) + "s")

print("Rendering time : " + str(R2 - R1) + "s")
...
print("All Total time : " + str(AT2 - AT1) + "s")
